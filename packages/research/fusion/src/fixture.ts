...
export const FUSION_FIXTURE_IDS = {
  ...
  evidence: {
    ...
    furukawaOpticalFiberFacilityInvestment2026: 'evidence-furukawa-optical-fiber-facility-investment-2026',
...
/**
 * Loads the fixture dataset from `data/dataset.json` — validated against
 * `@thunderdome/research-core`'s `ResearchDataset` schema — instead of constructing it inline in
 * TypeScript. See `scripts/migrateFixtureToJson.ts` for how that file was originally produced (a
 * mechanical, lossless dump of what this function used to build directly), and
 * `scripts/applyResearchUpdate.ts` for how it grows afterward from a JSON update file, with no
 * coding agent hand-editing this file required. The exported name/signature is unchanged from the
 * previous inline-construction version, so every consumer (bots, tests) is unaffected by this
 * move.
 *
 * Throws loudly on a validation failure rather than silently loading a partial/corrupted dataset
 * — a `data/dataset.json` that fails `@thunderdome/research-core`'s own validation is a bug that
 * must be fixed before anything else runs, not tolerated.
 */
export function createFusionFixtureDataset(): ResearchDataset {
  const datasetPath = resolve(dirname(fileURLToPath(import.meta.url)), '../data/dataset.json');
  const raw: unknown = JSON.parse(readFileSync(datasetPath, 'utf8'));
  const result = validateResearchDataset(raw);
  if (!result.ok) {
    throw new Error(
      `${datasetPath} failed research-core validation: ${formatValidationIssues(result.issues)}`,
    );
  }
  return result.value;
}
