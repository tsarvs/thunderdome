...
export const TICKERS: Record<string, TickerSource> = {
  ELMT: { yahooSymbol: 'ELMT', currency: 'USD', usdRate: 1 },
  ALM: { yahooSymbol: 'ALM', currency: 'USD', usdRate: 1 },
  OPTX: { yahooSymbol: 'OPTX', currency: 'USD', usdRate: 1 },
  GFUZ: { yahooSymbol: 'GFUZ', currency: 'USD', usdRate: 1 },
  KMT: { yahooSymbol: 'KMT', currency: 'USD', usdRate: 1 },
  AMSC: { yahooSymbol: 'AMSC', currency: 'USD', usdRate: 1 },
  FURUKAWA: { yahooSymbol: '5801.T', currency: 'JPY', usdRate: 1 / 153.605 },
  FUJIKURA: { yahooSymbol: '5803.T', currency: 'JPY', usdRate: 1 / 153.58 },
  SUMITOMO: { yahooSymbol: '5802.T', currency: 'JPY', usdRate: 1 / 153.58 },
  VITZRONEXTECH: { yahooSymbol: '488900.KQ', currency: 'KRW', usdRate: 1 / 1386.155 },
  FREEM: { yahooSymbol: 'FREEM.ST', currency: 'SEK', usdRate: 1 / 9.5868 },
};
...
async function fetchTicker(ticker: string, source: TickerSource, sinceDate: string, throughDate: string): Promise<FetchedBars> {
  ...
  if (!response.ok) {
    throw new Error(`${ticker}: Yahoo Finance request failed (${String(response.status)} ${response.statusText})`);
  }
  ...
  for (let i = 0; i < timestamps.length; i++) {
    ...
    const date = toCalendarDate(timestamps[i]!, result.meta.gmtoffset);
...
async function main(): Promise<void> {
  ...
  const through = typeof args.through === 'string' ? args.through : new Date().toISOString().slice(0, 10);
  ...
  const requestedTickers = typeof args.tickers === 'string' ? args.tickers.split(',') : queryTickers(store, identity);

  const results: FetchedBars[] = [];
  for (const ticker of requestedTickers) {
    const source = TICKERS[ticker];
    if (source === undefined) {
      console.error(`${ticker}: no Yahoo Finance mapping configured in this script's TICKERS table — skipped.`);
      continue;
    }
    const latestKnown = queryLatestKnownDate(store, identity, ticker);
    const sinceDate = latestKnown === null
      ? through // unknown ticker in this dataset — nothing to append from here, only via publishDatasetVersion
      : new Date(new Date(`${latestKnown}T00:00:00Z`).getTime() + 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
    if (latestKnown !== null && sinceDate > through) {
      console.log(`${ticker}: already known through ${latestKnown} — nothing new to fetch.`);
      continue;
    }
    if (latestKnown === null) {
      console.error(`${ticker}: not yet published in dataset "${datasetId}" version "${datasetVersion}" — publish it first, this script only grows an existing ticker.`);
      continue;
    }
    try {
      ...
      console.log(`${ticker}: fetched ${String(fetched.bars.length)} bar(s), ${fetched.bars[0]?.date} through ${fetched.bars.at(-1)?.date}`);
    } catch (error) {
  ...
  if (dryRun) {
    console.log('\n--dry-run: not appending. Bars found:', JSON.stringify(Object.fromEntries(results.map((r) => [r.ticker, r.bars])), null, 2));
    return;
  }
