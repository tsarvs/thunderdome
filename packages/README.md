...
| [`forward-match-store`](forward-match-store/)                       | Persisted `ForwardMatchRecord` read/write — one JSON file per forward match, no database (see ADR-0013). Atomic writes and a three-way found/not-found/corrupt load outcome.                                          | —                        |
