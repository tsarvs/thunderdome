...
| [`bot-sdk`](bot-sdk/)                                     | TypeScript SDK for bot authors: bot manifest schema, `runBot()` protocol client. Vendored into each bot's own tarball, not workspace-linked, since `bots/**` isn't a workspace member. | —                     |
| [`game-sdk`](game-sdk/)                                   | Helpers for game authors and the game manifest schema.                                                                                                                                 | —                     |
| [`registry`](registry/)                                   | Filesystem scan and validation of bot and game manifests — how the CLI resolves a bot/game id to real code.                                                                            | `bot-sdk`, `game-sdk` |
