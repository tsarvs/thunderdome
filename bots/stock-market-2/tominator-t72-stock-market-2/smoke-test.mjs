// Verifies tominator-t72-stock-market-2 against the real Docker runtime: trades a mean-reversion
// deviation in SYNTHETIC mode but holds through the identical deviation in HISTORICAL mode (no
// event), reacts to a real event in either mode, and proactively de-risks near its maintenance
// requirement. Requires: docker build -t thunderdome-tominator-t72-stock-market-2 .
import { DockerBotProcess, BotLifecycle, DEFAULT_RESOURCE_LIMITS } from '@thunderdome/runtime';

const IMAGE_TAG = 'thunderdome-tominator-t72-stock-market-2';
...
async function newMatch(matchId, config) {
  const botProcess = new DockerBotProcess({
    imageRef: IMAGE_TAG,
    matchId,
    participantId: 'tominator-t72-stock-market-2',
  ...
  await lifecycle.initialize(
    {
      ...
      gameVersion: '0.3.0',
      participantId: 'tominator-t72-stock-market-2',
      roster: ['tominator-t72-stock-market-2', 'opponent'],
      rngSeed: 'deadbeef',
      config,
    },
...
{
  const lifecycle = await newMatch('smoke-t72-synthetic-mr', { transactionFee: 0.001, risk: { allowShortSelling: false, borrowFeeAnnualized: 0.03 } });
...
{
  const lifecycle = await newMatch('smoke-t72-historical-hold', { mode: 'HISTORICAL', transactionFee: 0.001, risk: { allowShortSelling: false, borrowFeeAnnualized: 0.03 } });
...
// --- 3. HISTORICAL mode: still reacts to a genuine event. ---
{
  const lifecycle = await newMatch('smoke-t72-historical-event', { mode: 'HISTORICAL', transactionFee: 0.001, risk: { allowShortSelling: true, borrowFeeAnnualized: 0.03, initialMarginRatio: 0.5, maintenanceMarginRatio: 0.3 } });
...
{
  const lifecycle = await newMatch('smoke-t72-derisk', {
