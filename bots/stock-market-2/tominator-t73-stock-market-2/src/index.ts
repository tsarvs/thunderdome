/**
 * Tominator T-73 — Stock Market 2 — a deliberately more aggressive fork of
 * `tominator-t72-stock-market-2`, keeping its entire validated core (event-magnitude learning,
 * HISTORICAL-mode-trades-events-only, mean-reversion levered only in SYNTHETIC mode, cost-
 * awareness, a margin safety governor, borrow-cost-aware shorting, a portfolio-level stop-loss,
 * realized-volatility-scaled sizing, order-flow confirmation, and execution via a tight marketable
 * LIMIT order — see that file for the full rationale behind each, still true here) and changing
 * exactly what maximizes T-72's own real source of its biggest wins.
 *
 * Measured across many real matches, T-72's standout scores all came from the same place: a real
 * HISTORICAL-mode event (whose *direction* is always trustworthy — derived from real SEC filings)
 * landing while the bot held a large, leveraged position built the moment that event fired. Once
 * measured, it turned out T-72's own "learned magnitude" mechanism barely affects trade *size* at
 * all: `tradeFraction = clamp(magnitude * 40, 0.05, 0.85)`, and even T-72's smallest magnitude
 * prior (0.025) already saturates that clamp at its 0.85 ceiling on its own (0.85 / 40 = 0.02125)
 * — meaning T-72 already bets near-max size on literally every real event from the very first one,
 * before any learning happens. The ceiling itself, not the learning, is what actually controls win
 * size — so this fork raises the ceiling and the margin used against it, specifically for
 * event-driven trades only (the separate, more conservative mean-reversion caps are untouched,
 * since that's not what's producing the big wins and was already shown not to earn its own risk on
 * real data):
 *
 * 1. `MAX_TRADE_FRACTION_PER_EVENT`: 0.85 -> 0.97 — a real event can now command nearly all of
 *    portfolio value in one trade, cash-only.
 * 2. `BUYING_POWER_UTILIZATION_CAP_EVENT`: 0.6 -> 0.85 — a real event can now draw on most of
 *    current buying power, not merely 60% of it, when margin is enabled.
 * 3. A real event's own "never average down" guard is dropped entirely: a *second* confirming
 *    event can add to an already-open, currently-underwater position, since the event itself
 *    remains just as trustworthy regardless of a transient paper loss. The mean-reversion signal's
 *    own guard (the one that mattered on real data) is untouched.
 * 4. The margin/stop-loss backstops are loosened, not removed, so a real winning position has more
 *    room to run through ordinary volatility before being cut early: `MAINTENANCE_SAFETY_MULTIPLIER`
 *    1.5 -> 1.3, `STOP_LOSS_DRAWDOWN` 0.15 -> 0.2, `EXPECTED_HOLD_ROUNDS` (the short borrow-cost
 *    carry estimate) 8 -> 4.
 *
 * This is a real risk/reward trade: bigger, more concentrated bets mean bigger wins *and* bigger
 * losses when a bet goes wrong — this fork is deliberately higher-variance than T-72 by design,
...
/** Raised from T-72's 0.85 — see this file's own doc comment: this ceiling, not the learned
 * magnitude, is what actually controls a real event's trade size, so this is the direct lever for
 * maximizing T-72's own big-win mechanism. */
const MAX_TRADE_FRACTION_PER_EVENT = 0.97;
...
/** Raised from T-72's 0.6 — the other direct lever for maximizing a real event's win size when
 * margin is enabled: still short of the exchange's actual hard limit (so a single adverse move
 * doesn't immediately trigger a margin call), but using most rather than merely 60% of it. The
 * mean-reversion heuristic (SYNTHETIC mode only — see this file's own doc comment) keeps T-72's
 * original, more conservative cap. */
const BUYING_POWER_UTILIZATION_CAP_EVENT = 0.85;
const BUYING_POWER_UTILIZATION_CAP_MEAN_REVERSION = 0.35;
/** De-risk once equity falls under this multiple of the maintenance requirement — loosened from
 * T-72's 1.5 so a real winning position has more room to run through ordinary volatility before
 * being cut early, while still a real buffer above the exchange's own hard 1x maintenance line. */
const MAINTENANCE_SAFETY_MULTIPLIER = 1.3;
 ...
 * Shortened from T-72's 8 rounds: a smaller assumed carry cost means less of a real short's own
 * conviction gets discounted away before it's even opened. */
const EXPECTED_HOLD_ROUNDS = 4;
...
// whatever the current signal says. Loosened from T-72's 0.15 for the same "let a real winning
// position run" reason as the margin governor above — it only ever fires on a loss, never cuts a
// winning trade short, but a tighter threshold means more ordinary volatility gets mistaken for a
// real reason to exit. ---
const STOP_LOSS_DRAWDOWN = 0.2;
...
function decideAction(observation: Observation): Action {
  ...
  // "Never average down" only guards the mean-reversion signal now (this file's own doc comment,
  // point 3) — a real event stays trustworthy regardless of a transient paper loss, so a *second*
  // confirming event is allowed to add to an already-open, currently-underwater position.
  const blockAveragingDown = !isRealEvent && equityWasDeclining;

  if (sign > 0) {
    // Adding to an already-open long that's currently losing money, on the mean-reversion signal
    // only, is exactly the "average down" pattern this guard exists to block; opening fresh or
    // covering a short is never affected, and neither is a real event.
    const allowNewExposure = !(portfolio.shares > 0 && blockAveragingDown);
  ...
  let allowNewExposure = !(portfolio.shares < 0 && blockAveragingDown);
