/**
 * Momentum Stock Market — buys when the price just rose from the previous round, sells when it
 * just fell, holds when it's unchanged (or on round 0, before there's a previous round to
 * compare against). Trades a fixed quantity (capped at 10 shares, and by whatever's actually
 * affordable/owned) rather than sizing the trade to the size of the move.
 *
...
function decideAction(observation) {
  ...
  const previous = history.length >= 2 ? history[history.length - 2] : last;
