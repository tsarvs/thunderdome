...
export function computeTradingDecision(params: {
  ...
  const targetWeight = targetWeightForSignal(signal.level, signal.confidence, params.portfolioPolicy);
 ...
 * fund a same-round buy). v1: a SELL that opens/extends a short isn't debited from this budget
 * either — this budget models long-side buying power only; short-margin consumption is left to the
...
export function formatStrategyTrace(decision: TradingDecision): string {
  const lines = [
    `[fusion-fundamental-v1] ${decision.date ?? '(no date)'} ${decision.security}`,
