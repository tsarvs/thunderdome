...
export interface ValuationSensitivities {
  ...
  /** $/share of fusion option value per unit of a `supplier_capture` effect (a qualification/
   ...
   * `hypothesisConfidenceToFusionOptionPerShare`: a qualification or contract change is a materially
   * stronger evidence tier than a generic hypothesis-confidence shift (spec §10), so it should move
   * the option value more per unit of (magnitude * confidence) — but still only ever touches OPTION
   * value, never `fusionDerivedValue` (which stays governed solely by `../config.ts`'s
   * `FusionValuationAssumptions.supplierCapture`, a labeled bear/base/bull strategy assumption, not
   * something a single research effect silently overrides). */
  supplierCaptureToFusionOptionPerShare: number;
}
...
export function computeCompanyValuation(params: {
  ...
  for (const effect of params.effects) {
    ...
    if (effect.factor === 'supplier_capture' && effect.direction !== 'neutral') {
      ...
      fusionOptionValue = combineScenarios(
        fusionOptionValue,
        constantScenario(sign * weight * params.sensitivities.supplierCaptureToFusionOptionPerShare),
        (a, b) => a + b,
      );
    }
