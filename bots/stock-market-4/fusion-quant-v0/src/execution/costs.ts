 ...
 */
export function estimateOrderCost(params: { ticker: string; notionalUsd: number; policy: ExecutionCostPolicy }): OrderCostEstimate {
  const averageDailyVolumeUsd = params.policy.averageDailyVolumeUsdByTicker.get(params.ticker) ?? params.policy.defaultAverageDailyVolumeUsd;
  const participationRate = averageDailyVolumeUsd > 0 ? Math.abs(params.notionalUsd) / averageDailyVolumeUsd : 0;
  const marketImpactUsd = params.policy.impactCoefficient * participationRate * Math.abs(params.notionalUsd);
  const totalCostUsd = params.policy.flatFeeUsd + marketImpactUsd;
  const totalCostBps = params.notionalUsd !== 0 ? (totalCostUsd / Math.abs(params.notionalUsd)) * 10_000 : 0;
