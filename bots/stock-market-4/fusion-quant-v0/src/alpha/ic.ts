...
export function computeEnsembleWeights(
  ...
  const rawWeight = new Map(presentFactors.map((factor) => [factor, measuredWeight.get(factor) ?? averageMeasured]));
...
export function combineAlphaEnsemble(
  ...
  return { expectedReturn: weightedValue / totalWeight, confidence: weightedConfidence / totalWeight };
}
