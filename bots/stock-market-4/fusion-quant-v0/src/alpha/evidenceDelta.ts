...
export function computeEvidenceDeltaAlpha(params: {
  ...
  const relevant = params.effects.filter(
    (effect) => effect.factor === 'manufacturing_capability' || effect.factor.startsWith('hypothesis:'),
  );
  if (relevant.length === 0) {
    return { factor: 'evidence_delta', ticker: params.ticker, date: params.date, value: 0, confidence: 0 };
  }
