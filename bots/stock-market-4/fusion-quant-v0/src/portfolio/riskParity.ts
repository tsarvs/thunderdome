import { effectiveCorrelationKey, type PortfolioConstructionInput, type PortfolioConstructionPolicy, type PortfolioConstructionResult, type PortfolioStrategy } from './types.js';
...
export const riskParityStrategy: PortfolioStrategy = {
  ...
  construct(input: PortfolioConstructionInput, policy: PortfolioConstructionPolicy): PortfolioConstructionResult {
    ...
    const fallbackVol = knownVols.length > 0 ? knownVols.reduce((sum, v) => sum + v, 0) / knownVols.length : 0.02;

    const volByTicker = new Map(active.map((a) => [a.ticker, a.dailyVolatility && a.dailyVolatility > 0 ? a.dailyVolatility : fallbackVol]));
    ...
    for (const a of active) {
      ...
      const correlations = others.map(
        (b) => input.effectiveCorrelationByPair.get(effectiveCorrelationKey(a.ticker, b.ticker)) ?? 0,
      );
      avgCorrelationByTicker.set(a.ticker, correlations.reduce((sum, c) => sum + Math.max(0, c), 0) / others.length);
    }

    const riskProxyByTicker = new Map(
      active.map((a) => [
        a.ticker,
        volByTicker.get(a.ticker)! * (1 + avgCorrelationByTicker.get(a.ticker)! * DEFAULT_RISK_PARITY_CORRELATION_PENALTY),
      ]),
    );
    const inverseRiskByTicker = new Map(active.map((a) => [a.ticker, 1 / riskProxyByTicker.get(a.ticker)!]));
    const totalInverseRisk = [...inverseRiskByTicker.values()].reduce((sum, v) => sum + v, 0);

    for (const a of active) {
      const share = (inverseRiskByTicker.get(a.ticker)! / totalInverseRisk) * policy.totalGrossBudgetPct;
