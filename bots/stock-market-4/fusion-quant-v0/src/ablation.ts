...
const RESEARCH_FACTORS = new Set(['evidence_delta', 'commercialization', 'valuation_gap', 'market_implied']);
...
export const ALL_ALPHA_FACTORS: string[] = ['evidence_delta', 'commercialization', 'valuation_gap', 'market_implied', 'momentum'];
...
export function activeFactorsForMode(mode: ResearchAblationMode): string[] {
  ...
  if (mode === 'research_only') return ALL_ALPHA_FACTORS.filter((factor) => !PRICE_FACTORS.has(factor));
...
export function applyAblation(signals: AlphaSignal[], mode: ResearchAblationMode): AlphaSignal[] {
  ...
  if (mode === 'research_only') return signals.filter((signal) => !PRICE_FACTORS.has(signal.factor));
