...
describe('computeValuationGapAlpha', () => {
  it('is the fair-value-vs-price fractional gap', () => {
    const result = computeValuationGapAlpha({ ticker: 'X', date: null, fairValuePerShare: 11, marketPriceDollars: 10 });
  ...
  it('is silent (zero confidence) for a non-positive market price rather than dividing by zero/negative', () => {
    const result = computeValuationGapAlpha({ ticker: 'X', date: null, fairValuePerShare: 11, marketPriceDollars: 0 });
