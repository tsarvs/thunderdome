...
describe('computeMomentumAlpha', () => {
  it('is silent when there is not enough history for the requested window', () => {
    const result = computeMomentumAlpha({ ticker: 'X', date: null, priceHistory: [], currentPriceDollars: 10, windowDays: 10 });
  ...
  it('is the trailing return, with confidence scaled by how much of the window is covered', () => {
    ...
    const result = computeMomentumAlpha({ ticker: 'X', date: null, priceHistory: history, currentPriceDollars: 11, windowDays: 5 });
