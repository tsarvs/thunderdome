...
const POLICY: PortfolioConstructionPolicy = { activationThreshold: 0.01, totalGrossBudgetPct: 0.6, maxPositionWeight: 0.5 };
...
describe('inverseVolatilityStrategy', () => {
  it('sizes a calmer security larger than a wilder one', () => {
    ...
    expect(result.weightByTicker.get('CALM')!).toBeGreaterThan(result.weightByTicker.get('WILD')!);
  });
  ...
  it('falls back to the average measured inverse-volatility for a security with no volatility reading yet', () => {
    ...
    expect(result.weightByTicker.get('KNOWN')).toBeCloseTo(result.weightByTicker.get('UNKNOWN')!);
  });

  it('is fully in cash when nothing activates', () => {
    const result = inverseVolatilityStrategy.construct({ alphas: [{ ticker: 'A', expectedReturn: 0, confidence: 1, dailyVolatility: 0.02 }], effectiveCorrelationByPair: new Map() }, POLICY);
