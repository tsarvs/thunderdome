...
export const DEFAULT_FUSION_FUNDAMENTAL_CONFIG: FusionFundamentalConfig = {
  ...
  volatilitySizing: {
    // 2%: roughly the midpoint of confidenceCalibration's own calm(1%)/chaotic(5%) range — an
    // illustrative starting point (spec §28), not swept/tuned to this window.
    referenceDailyVolatility: 0.02,
    // Never size below half or above 1.5x the raw target — bounds the effect so a single very
    // calm or very choppy reading can't dominate the position size outright.
    minScaleFactor: 0.5,
    maxScaleFactor: 1.5,
  },
  stopLoss: {
    // 25%: an illustrative starting point (spec §28, not swept/tuned to this window), set wider
    // than a typical stop given this basket's own elevated realized volatility (several securities
    // >5% daily vol — a tighter stop would risk getting stopped out by ordinary noise, not a real
    // adverse move).
    stopLossPct: 0.25,
  },
};
