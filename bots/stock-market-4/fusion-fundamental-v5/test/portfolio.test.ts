...
import { buildOrders, isStopLossTriggered, scaleTargetWeightForVolatility, spendableCentsFor, targetWeightForSignal } from '../src/portfolio.js';
...
describe('scaleTargetWeightForVolatility (v4)', () => {
  const volPolicy = { referenceDailyVolatility: 0.02, minScaleFactor: 0.5, maxScaleFactor: 1.5 };

  it('passes HOLD (undefined) and flatten (0) targets through unchanged', () => {
    expect(scaleTargetWeightForVolatility(undefined, 0.05, volPolicy, 0.25)).toBeUndefined();
    expect(scaleTargetWeightForVolatility(0, 0.05, volPolicy, 0.25)).toBe(0);
  });

  it('passes the target through unchanged with no volatility reading yet', () => {
    expect(scaleTargetWeightForVolatility(0.2, undefined, volPolicy, 0.25)).toBe(0.2);
  });

  it('scales DOWN a target for a choppier-than-reference security, within the floor', () => {
    // 0.08 daily vol vs. 0.02 reference -> raw factor 0.25, floored at minScaleFactor 0.5.
    expect(scaleTargetWeightForVolatility(0.2, 0.08, volPolicy, 0.25)).toBeCloseTo(0.1); // 0.2 * 0.5
  });

  it('scales UP a target for a calmer-than-reference security, within the cap, then re-clamps to maxPositionWeight', () => {
    // 0.005 daily vol vs. 0.02 reference -> raw factor 4, capped at maxScaleFactor 1.5 -> 0.2*1.5=0.3,
    // then re-clamped to maxPositionWeight (0.25).
    expect(scaleTargetWeightForVolatility(0.2, 0.005, volPolicy, 0.25)).toBeCloseTo(0.25);
  });

  it('preserves sign when scaling a short (negative) target', () => {
    expect(scaleTargetWeightForVolatility(-0.2, 0.08, volPolicy, 0.25)).toBeCloseTo(-0.1);
  });
});

describe('isStopLossTriggered (v5)', () => {
  it('is false with no position (zero shares)', () => {
    expect(isStopLossTriggered(0, 2000, 10, 0.25)).toBe(false);
  });

  it('triggers for a long once price falls far enough below entry', () => {
    // entry $20, now $14 -> down 30%, past a 25% stop.
    expect(isStopLossTriggered(100, 2000, 14, 0.25)).toBe(true);
  });

  it('does not trigger for a long within the threshold', () => {
    // entry $20, now $16 -> down 20%, within a 25% stop.
    expect(isStopLossTriggered(100, 2000, 16, 0.25)).toBe(false);
  });

  it('triggers for a short once price rises far enough above entry (mirror-image adverse move)', () => {
    // entry $20 (short), now $26 -> up 30% against the short, past a 25% stop.
    expect(isStopLossTriggered(-100, 2000, 26, 0.25)).toBe(true);
  });

  it('does not trigger for a short when price falls (a gain, not an adverse move)', () => {
    expect(isStopLossTriggered(-100, 2000, 10, 0.25)).toBe(false);
  });
});
