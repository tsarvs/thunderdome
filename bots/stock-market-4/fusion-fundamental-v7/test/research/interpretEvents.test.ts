...
describe('interpretResearchDelta — supplier-capture handling', () => {
      ...
      relationships: [
        {
          ...
          fromEntityId: TARGET,
          toEntityId: ARC,
          states: [{ status: 'qualified', recordedAt: 't2', effectiveFrom: 't2', evidenceIds: [], confidence: { value: 0.8 } }],
        },
      ],
    };
    const delta = computeResearchDelta(previous, current);
  ...
  it('produces a supplier_capture effect for a STATUS TRANSITION (qualification track -> production contract)', () => {
    const previousRelationship = {
      ...
      fromEntityId: TARGET,
      toEntityId: ARC,
      states: [{ status: 'qualification track', recordedAt: 't1', effectiveFrom: 't1', evidenceIds: [] }],
    };
      ...
      relationships: [
        {
          ...
          states: [...previousRelationship.states, { status: 'production contract', recordedAt: 't2', effectiveFrom: 't2', evidenceIds: [], confidence: { value: 1 } }],
        },
      ],
    };
    const delta = computeResearchDelta(previous, current);
