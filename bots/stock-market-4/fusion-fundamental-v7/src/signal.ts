...
export function computeSignal(params: {
  ...
  previousLevel?: SignalLevel | undefined;
  /** An ADDITIONAL confidence factor from outside the research-effect pipeline — currently used
   * for the market-implied-valuation check (`decision.ts`): a security whose current price already
   * implies MORE fusion success than even this strategy's own bull case gets a reduced multiplier
   * here (spec §39's "valuation blindness" guard — never buy an already-priced-in narrative
   * regardless of price), and one implying LESS than even the bear case gets a boosted one (the
   * asymmetric-upside case, spec §14). Folded in HERE (not just multiplied onto the final
   * `confidence` afterward) specifically so it can move `signalScore` enough to cross a
   * `SignalLevel` bucket boundary — a confidence adjustment applied only to the outer, reported
   * number never actually changes what gets traded. Defaults to `1` (no adjustment). */
  externalConfidenceMultiplier?: number;
}): Signal {
  const valuationGap = computeValuationGap(params.fairValuePerShare, params.marketPrice);
  const confidence = computeSignalConfidence(params.effects) * (params.externalConfidenceMultiplier ?? 1);
