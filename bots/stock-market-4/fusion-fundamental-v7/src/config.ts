...
export interface FusionFundamentalConfig {
  ...
  /** v4-only: inverse-volatility ("vol targeting" / risk-parity) position-size scaling — see
   * `VolatilitySizingPolicy`'s own doc comment. */
  volatilitySizing: VolatilitySizingPolicy;
  /** v5-only: a trailing stop-loss, overriding whatever the signal says — see `StopLossPolicy`'s
   * own doc comment. */
  stopLoss: StopLossPolicy;
  /** v6-only: correlation-aware position-size scaling — see `CorrelationSizingPolicy`'s own doc
   * comment. */
  correlationSizing: CorrelationSizingPolicy;
  /** The portfolio-level thesis-group cap (spec §20/§42) — see `portfolio/optimizer.ts`'s own doc
   * comment. Distinct from `correlationSizing` above: that scales from real trailing PRICE
   * correlation; this scales from shared RESEARCH exposure (the same causal graph two securities
   * might both sit on), which can catch a concentrated bet even before its price history shows any
   * correlation at all (e.g. two securities newly added to the universe with no shared trading
   * history yet, but an evidenced shared supplier/program relationship already on record). */
  thesisGroupCap: ThesisGroupCapPolicy;
  /** How a `supplier_capture`/market-implied-valuation finding actually reaches a trading
   * decision — see `AdvancedSignalPolicy`'s own doc comment. This is what makes this bot a
   * genuinely different strategy from `fusion-fundamental-v6`, not just v6 plus two dollar-value
   * nudges too small to ever cross a `SignalThresholds` bucket boundary. */
  advancedSignal: AdvancedSignalPolicy;
}

/**
 * Two mechanisms, both designed to actually move a `SignalLevel` bucket rather than just nudge a
 * dollar-denominated fair value that can be swamped by a security's own base price (the gap this
 * config exists to close — confirmed empirically: a real historical head-to-head against
 * fusion-fundamental-v6 produced BYTE-IDENTICAL portfolios, because every `supplier_capture`
 * dollar-value effect that fired was too small to cross a threshold).
 */
export interface AdvancedSignalPolicy {
  /** A `supplier_capture` effect (see `research/interpretEvents.ts`) shifts THIS security's
   * `SignalThresholds` themselves for the round it fires, by `magnitude * confidence *
   * supplierCaptureThresholdShift` — an upgrade (positive direction) moves `strongBuyThreshold`/
   * `buyThreshold` DOWN (easier to earn a BUY/STRONG_BUY), a downgrade (negative direction) moves
   * `sellThreshold`/`strongSellThreshold` UP (easier to earn a SELL/STRONG_SELL) — see
   * `decision.ts`'s `adjustThresholdsForSupplierCapture`. Deliberately a SEPARATE mechanism from
   * `ValuationSensitivities.supplierCaptureToFusionOptionPerShare` (kept, not replaced) — that
   * still contributes real dollars to fair value; this makes a confirmed qualification/contract-
   * tier change easier to act on regardless of how large a share of the security's own price that
   * dollar amount represents. */
  supplierCaptureThresholdShift: number;
  /** Applied to `signal.ts`'s `externalConfidenceMultiplier` when
   * `MarketImpliedExpectations.impliedCaptureVsRange === 'above_bull_case'` — spec §39's
   * "valuation blindness" guard: the market is already pricing in MORE fusion success than even
   * this strategy's own most optimistic scenario, so conviction should shrink, not just get
   * reported. Always `<= 1` (a penalty, never a boost). */
  marketImpliedOptimismConfidencePenalty: number;
  /** Applied when `impliedCaptureVsRange === 'below_bear_case'` — the asymmetric-upside case spec
   * §14 describes (the market prices in LESS than even this strategy's own worst case). Always
   * `>= 1` (a boost, never a penalty) and capped defensively in `decision.ts` so it can never turn
   * a HOLD into a STRONG_BUY on its own — a real finding should widen conviction, not manufacture
   * it from nothing (spec §28). */
  marketImpliedUpsideConfidenceBoost: number;
}

/**
 * v6-only. A real portfolio-construction practice: several tracked securities in the same supply
 * chain (e.g. ELMT/ALM/FREEM, all tungsten-adjacent) can move together closely enough that holding
 * all of them near max size isn't real diversification — it's one concentrated bet on "tungsten
 * demand" wearing three tickers. This scales a security's target weight down when its recent
 * returns are POSITIVELY correlated with the rest of the currently-priced book, computed from
 * real trailing daily returns (`correlation.ts`) — no assumed/hardcoded correlation matrix, no
 * look-ahead. Negative correlation is deliberately NEVER penalized: an anti-correlated pair is a
 * real hedge (a genuine diversification benefit), not a concentrated bet, so only positive
 * correlation is averaged into the scale factor. See `decision.ts`'s `computeTradingDecisions`,
 * the only place with visibility into every tracked security's history at once (a single
 * security's own decision has no way to know what else is in the book).
 */
export interface CorrelationSizingPolicy {
  /** Trailing trading days for the return series correlation is computed from. */
  windowDays: number;
  /** How aggressively average POSITIVE correlation scales a target down — e.g. 0.5 means a
   * security perfectly correlated (avg corr = 1) with the rest of the book gets scaled to 50% of
   * its raw target (before `minScaleFactor` flooring). Negative correlation never contributes. */
  concentrationPenalty: number;
  /** Never scale below this floor, no matter how correlated — a hard floor, same shape as
   * `VolatilitySizingPolicy.minScaleFactor`. */
  minScaleFactor: number;
}

/**
 * v5-only. A standard, practical real-world risk control used by both discretionary and
 * systematic traders alike: exit a position once it's moved far enough against its own entry
 * price, REGARDLESS of what the valuation model currently believes. This is deliberately NOT a
 * model-driven decision — it's an unconditional backstop for "the model might be wrong, or right
 * too late," independent of signal level, trend filter, or volatility sizing. Uses only
 * `PortfolioObservation.positions[].averageEntryPriceCents`, real state the game itself already
 * tracks — no invented bookkeeping.
 *
 * A long is stopped out once price has fallen `stopLossPct` below entry; a short is stopped out
 * once price has risen `stopLossPct` above entry (the mirror-image adverse move). Checked AFTER
 * the trend filter and volatility scaling, as the final word — a stop-loss always wins over a
 * "the signal still likes this" holdout.
 */
...
/**
 * v4-only. Standard real-world risk-parity / volatility-targeting practice: size a position
 * inversely to its OWN recent realized volatility, so a calm security and a wild one don't get
 * the same fixed target weight just because they both earned the same signal level. Without this,
 * `strongBuyTargetWeight`/etc. size every security identically regardless of how much risk a
 * dollar of that security actually carries — e.g. GFUZ (a newly-listed, highly volatile micro-cap)
 * and ALM (a comparatively calmer miner) would get the same 20% STRONG_BUY allocation even though
 * a dollar of GFUZ swings far more day to day.
 *
 * `referenceDailyVolatility` is the volatility level that leaves a target weight UNCHANGED (scale
 * factor 1.0); a calmer security scales UP (within `maxScaleFactor`), a choppier one scales DOWN
 * (within `minScaleFactor`). Applied multiplicatively to whatever `targetWeightForSignal` (and
 * v3's trend filter) already produced, then re-clamped to `maxPositionWeight` — see
 * `portfolio.ts`'s `scaleTargetWeightForVolatility`.
 */
export interface VolatilitySizingPolicy {
  referenceDailyVolatility: number;
  minScaleFactor: number;
...
export const DEFAULT_FUSION_FUNDAMENTAL_CONFIG: FusionFundamentalConfig = {
  ...
  signal: {
    ...
    // v6: 0.01, not v2/v3's 0.03 — see games/stock-market-4/scripts/optimizeFusionFundamentalV6.ts.
    // 0.03 was tuned for v3's OWN character; v6's added vol-sizing/correlation-sizing/stop-loss
    // stack changes how the underlying signal behaves round to round, so that value wasn't
    // guaranteed to still be right — re-swept specifically for v6's full combined behavior rather
    // than assumed inherited. 0.01 clearly won on return, Sharpe, AND volatility simultaneously
    // (not a tradeoff) against every other value tried (0, 0.02, 0.03, 0.05, 0.08).
    hysteresisBand: 0.01,
  },
  portfolio: {
  ...
  volatilitySizing: {
    // Sweep-tuned (games/stock-market-4/scripts/optimizeFusionFundamentalV6.ts, real July-August
    // 2026 window, a balanced return/drawdown/volatility/Sharpe composite score): the original 2%
    // guess (roughly the midpoint of confidenceCalibration's own calm/chaotic range) turned out to
    // be systematically too tight for this basket's real volatility — most securities here
    // regularly exceed 2% daily vol, so nearly every position was getting scaled DOWN almost every
    // round, dragging v6's return/Sharpe well below v3/v4/v5 without a proportional drawdown
    // benefit. 6% (near the confidenceCalibration's own "chaotic" ceiling) makes the scaling much
    // less aggressive by default, only meaningfully kicking in for genuinely extreme volatility.
    referenceDailyVolatility: 0.06,
    // Sweep-tuned follow-up pass (same script): 0.5 was STILL over-dampening exactly the volatile
    // movers this basket's returns are concentrated in (the same value/momentum-vs-vol-targeting
    // tension noted when v4 first shipped), even after the reference-point fix above. 1.0 means
    // volatility sizing can only ever scale a target UP (toward 1.5x, for a calm reading) and
    // never down — a real, if asymmetric, choice: reward genuinely calm opportunities with extra
    // size, but stop penalizing the volatile names actually driving this strategy's edge. Clearly
    // beat every value tried at the old two-sided 0.5 floor on the same balanced composite score.
    minScaleFactor: 1.0,
    maxScaleFactor: 1.5,
  },
  stopLoss: {
    // Sweep-tuned (same script/window as above): 25% was getting triggered often enough on this
    // basket's ordinary volatility to cut winners short before the underlying signal played out;
    // 40% lets a position ride out more of the day-to-day noise while still catching a genuinely
    // sustained adverse move.
    stopLossPct: 0.4,
  },
  correlationSizing: {
    windowDays: 20, // same window as confidenceCalibration's own, for consistency
    // Illustrative starting points (spec §28, not swept/tuned): a perfectly correlated security
    // scales down by up to half, never below a quarter of its raw target regardless of how
    // correlated it is.
    concentrationPenalty: 0.5,
    minScaleFactor: 0.25,
  },
  thesisGroupCap: {
    // Illustrative starting point (spec §28, not swept/tuned): two securities sharing even ONE
    // exposure id (one architecture entity, one bottleneck hypothesis, ...) are treated as the
    // same commercialization bet — deliberately permissive, since spec §42's whole point is to
    // CATCH understated concentration, not require an implausibly large shared footprint first.
    minSharedExposureIds: 1,
    // Combined target weight across a thesis group is capped at 1.5x a single position's own max
    // (maxPositionWeight above) — enough room for two genuinely-related names to both be held,
    // but not for a shared bet to silently occupy two full-sized independent slots.
    maxGroupWeight: 0.25 * 1.5,
  },
  advancedSignal: {
    // Illustrative starting point (spec §28, not swept/tuned): a maximally-confident (magnitude=1,
    // confidence=1) supplier_capture change shifts the relevant threshold by a full 8 percentage
    // points — comparable to the distance between adjacent thresholds themselves (e.g.
    // buyThreshold 0.05 -> strongBuyThreshold 0.15 is a 10-point gap), so a genuine qualification/
    // contract-tier change can plausibly cross a bucket boundary on its own, which
    // `ValuationSensitivities.supplierCaptureToFusionOptionPerShare` alone could not.
    supplierCaptureThresholdShift: 0.08,
    // Halves conviction when the market already prices in more than this strategy's own bull
    // case — a real caution signal, not a rounding adjustment.
    marketImpliedOptimismConfidencePenalty: 0.5,
    // A modest boost (not doubled) when the market prices in less than even this strategy's own
    // bear case — real, but deliberately smaller in magnitude than the penalty above, since
    // "the market may be wrong" is a weaker claim than "our own model already agrees this is a
    // caution case."
    marketImpliedUpsideConfidenceBoost: 1.2,
  },
};
