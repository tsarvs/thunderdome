...
describe('market reaction vs. modeled fair value (spec §12/§20)', () => {
  ...
  it('the same positive research event produces different trade signals depending purely on market price, never trading on news direction alone', () => {
    ...
    // -0.2625 clears v1's strongSellThreshold (-0.25) — even more clearly bearish than plain SELL,
    // which only strengthens this test's actual point (still purely price-driven, not news-driven).
    expect(expensive.level).toBe('STRONG_SELL');
  });
});
