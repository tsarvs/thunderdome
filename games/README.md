...
| [`stock-market`](stock-market/)               | `@thunderdome/game-stock-market`        | 2-10    | No            | `round-robin`                       | A simulated single-stock market — every bot trades simultaneously each round, trying to end with the highest portfolio value; the first game with a hidden per-match variable (the fundamental value) that isn't a hand of cards. |
