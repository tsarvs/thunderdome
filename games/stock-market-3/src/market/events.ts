...
export function buildCalendar(
  ...
  for (let round = 7; round < totalRounds; round += RELEASE_CADENCE_ROUNDS) {
    const indicator =
      ECONOMIC_RELEASE_INDICATORS[releaseIndex % ECONOMIC_RELEASE_INDICATORS.length] ??
      ECONOMIC_RELEASE_INDICATORS[0];
...
const INDICATOR_PROFILES: Record<
  EconomicReleaseIndicator,
  { baseline: number; scale: number; noise: number }
> = {
...
const INDICATOR_FACTOR: Record<
  EconomicReleaseIndicator,
  'GROWTH' | 'INFLATION' | 'INTEREST_RATES' | 'COMMODITY_PRICES'
> = {
  GDP_GROWTH: 'GROWTH',
...
export function forecastIndicatorReading(
...
export function deriveIndicatorReading(
  ...
  return (
    Math.round(
      (profile.baseline + profile.scale * economicFactorValue + gaussian(rng) * profile.noise) *
        100,
    ) / 100
  );
}
 ...
 */
export function earningsSurpriseImpact(
  reported: ReportedFinancials,
  consensus: ReportedFinancials,
): number {
  const epsSurprise =
    (reported.epsCents - consensus.epsCents) / Math.max(1, Math.abs(consensus.epsCents));
  const revenueSurprise =
    (reported.revenueCents - consensus.revenueCents) /
    Math.max(1, Math.abs(consensus.revenueCents));
